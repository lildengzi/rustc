...
- `String` 会交出指向「未来会被释放缓冲区」的 `&'static str`，`for c in s` 直接迭代已释放的缓冲区——跨 drop 持有或迭代就是悬垂引用；
...
**不是模拟，是真的**：垃圾读取、错误结果、数据竞争、`free(): double free detected` 崩溃，样样都有。本人对使用后果不承担任何责任。想围观每个陷阱爆炸，跑一遍 `examples/` 下的二进制即可。
